...
QRange::QRange(double xmin, double xmax, double ymin, double ymax, double zmin, double zmax):
    min(xmin, ymin, zmin),
    max(xmax, ymax, zmax)
{}
...
void QAxis::drawCoordinate() const
{
    if(mXTicks.isEmpty()) return;
    if(mYTicks.isEmpty()) return;
    if(mZTicks.isEmpty()) return;
...
//        mPlot->draw3DLine(QVector3D(0,0,0), QVector3D(mXTicks[mXTicks.size()-1],0,0),2,mLabelColor);
        mPlot->draw3DLine(QVector3D(0,0,0), QVector3D(10,0,0),2,mLabelColor);
        mPlot->renderTextAtWorldCoordinates(QVector3D(12,0,0),mXLabel,mLabelFont,mLabelColor);
    }
    glPopMatrix();
}
...
void QPlot3D::paintGL()
{
    ...
    // DRAW COORDINATE
    mXAxis.drawCoordinate();
    mYAxis.drawCoordinate();
    mZAxis.drawCoordinate();
    ...
    {
        drawTextBox(10,height()-10,QString("yaw: %1 pit: %2").arg(((azimuth()>180)?(azimuth()-360):azimuth()),3,'f',1).arg(elevation(),3,'f',1));
    }
}
...
void QPlot3D::drawTextBox(int x, int y, QString string, QFont font)
{
    ...
    glEnable(GL_BLEND);
    enable2D();
    Draw2DPlane(QVector2D(x-5,y+5), QVector2D(x+5+textWidth, y-textHeight-5), mBackgroundColor);
    disable2D();
    glDisable(GL_BLEND);

    glColor4f(0,0,0,1);
    renderTextAtScreenCoordinates(x,y,string,font,xAxis().mLabelColor);
}
...
//    return q.rotatedVector(-QVector3D(mTranslate.x()/mScale.x(),mTranslate.y()/mScale.y(),mTranslate.z()/mScale.z()));
  return tObjectCenter + q.rotatedVector(-QVector3D(mTranslate.x()/mScale.x(),mTranslate.y()/mScale.y(),mTranslate.z()/mScale.z()));
}

void QPlot3D::renderTextAtWorldCoordinates(const QVector3D& vec, QString str, QFont font, QColor color)
{
    QVector3D sVec = toScreenCoordinates(vec);
    renderTextAtScreenCoordinates(sVec.x(),sVec.y(),str,font,color);
}

void QPlot3D::renderTextAtScreenCoordinates(int x, int y, QString str, QFont font, QColor color)
{
    setFont(font);
    qglColor(color);
    renderText(x,y,str);
...
void QPlot3D::draw3DLine(QVector3D from, QVector3D to, double lineWidth, QColor color)
{
    ...
    float colorVec[] =
    {
        (float)color.redF(), (float)color.greenF(), (float)color.blueF(), 0.0,
        (float)color.redF(), (float)color.greenF(), (float)color.blueF(), 0.0,
        (float)color.redF(), (float)color.greenF(), (float)color.blueF(), 1.0,
        (float)color.redF(), (float)color.greenF(), (float)color.blueF(), 1.0,
        (float)color.redF(), (float)color.greenF(), (float)color.blueF(), 0.0,
        (float)color.redF(), (float)color.greenF(), (float)color.blueF(), 0.0,
    };
